...
const run = async () => {
  ...
  const tournamentName = `Load Tournament ${timestamp.toISOString().slice(0, 19).replace(/[:T]/g, '-')}`;
  ...
  const [underReviewCount, approvedCount, totalGames] = await Promise.all([
  ...
  console.log('[seed:large-tournament] done');
...
run()
  .catch((error) => {
    console.error('[seed:large-tournament] failed', error.message);
    process.exitCode = 1;
  })
  .finally(async () => {
